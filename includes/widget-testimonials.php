...
class AI_Widget_Testimonials extends WP_Widget {
	protected $ai_widget_cssclass;
	protected $ai_widget_description;
	protected $ai_widget_idbase;
	protected $ai_widget_title;
	...
	public function __construct() {
		...
		$this->ai_widget_cssclass 	= 'widget_ai_testimonials';
		$this->ai_widget_description = __( 'Recent testimonials on your site.', 'ai-testimonials' );
		$this->ai_widget_idbase 		= 'ai_testimonials';
		$this->ai_widget_title 		= __( 'Testimonials', 'ai-testimonials' );

		/* Widget settings. */
		$widget_ops = array( 'classname' => $this->ai_widget_cssclass, 'description' => $this->ai_widget_description );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => $this->ai_widget_idbase );

		/* Create the widget. */
		$this->WP_Widget( $this->ai_widget_idbase, $this->ai_widget_title, $widget_ops, $control_ops );
	} // End __construct()
	...
	public function widget( $args, $instance ) {
		...
		do_action( $this->ai_widget_cssclass . '_top' );
		...
		ai_testimonials( $args );

		// Add actions for plugins/themes to hook onto.
		do_action( $this->ai_widget_cssclass . '_bottom' );
	} // End widget()
...
?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title (optional):', 'ai-testimonials' ); ?></label>
			<input type="text" name="<?php echo $this->get_field_name( 'title' ); ?>"  value="<?php echo $instance['title']; ?>" class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" />
		</p>
		<!-- Widget Limit: Text Input -->
		<p>
			<label for="<?php echo $this->get_field_id( 'limit' ); ?>"><?php _e( 'Limit:', 'ai-testimonials' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'per_row' ); ?>"><?php _e( 'Testimonials Per Row:', 'ai-testimonials' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'size' ); ?>"><?php _e( 'Image Size (in pixels):', 'ai-testimonials' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'orderby' ); ?>"><?php _e( 'Order By:', 'ai-testimonials' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'order' ); ?>"><?php _e( 'Order Direction:', 'ai-testimonials' ); ?></label>
		...
		<!-- Widget Display Author: Checkbox Input -->
       	<p>
        	...
        	<label for="<?php echo $this->get_field_id( 'display_author' ); ?>"><?php _e( 'Display Author', 'ai-testimonials' ); ?></label>
	   	</p>
		<!-- Widget Display Avatar: Checkbox Input -->
       	<p>
        	...
        	<label for="<?php echo $this->get_field_id( 'display_avatar' ); ?>"><?php _e( 'Display Avatar', 'ai-testimonials' ); ?></label>
	   	</p>
       	...
       	<p>
        	...
        	<label for="<?php echo $this->get_field_id( 'display_url' ); ?>"><?php _e( 'Display URL', 'ai-testimonials' ); ?></label>
	   	</p>
	   	<!-- Widget Category: Select Input -->
		<p>
			<label for="<?php echo $this->get_field_id( 'category' ); ?>"><?php _e( 'Category:', 'ai-testimonials' ); ?></label>
			<?php
				$dropdown_args = array( 'taxonomy' => 'testimonial-category', 'class' => 'widefat', 'show_option_all' => __( 'All', 'ai-testimonials' ), 'id' => $this->get_field_id( 'category' ), 'name' => $this->get_field_name( 'category' ), 'selected' => $instance['category'] );
				wp_dropdown_categories( $dropdown_args );
			?>
		</p>
		<!-- Widget ID: Text Input -->
		<p>
			<label for="<?php echo $this->get_field_id( 'specific_id' ); ?>"><?php _e( 'Specific ID (optional):', 'ai-testimonials' ); ?></label>
		...
		<p><small><?php _e( 'Display a specific testimonial, rather than a list.', 'ai-testimonials' ); ?></small></p>
<?php
	} // End form()
	...
	protected function get_orderby_options () {
		return array(
					'none' => __( 'No Order', 'ai-testimonials' ),
					'ID' => __( 'Entry ID', 'ai-testimonials' ),
					'title' => __( 'Title', 'ai-testimonials' ),
					'date' => __( 'Date Added', 'ai-testimonials' ),
					'menu_order' => __( 'Specified Order Setting', 'ai-testimonials' ),
					'rand' => __( 'Random Order', 'ai-testimonials' )
					);
	...
	protected function get_order_options () {
		return array(
					'ASC' => __( 'Ascending', 'ai-testimonials' ),
					'DESC' => __( 'Descending', 'ai-testimonials' )
					);
	} // End get_order_options()
} // End Class

/* Register the widget. */
add_action( 'widgets_init', create_function( '', 'return register_widget("AI_Widget_Testimonials");' ), 1 );
?>